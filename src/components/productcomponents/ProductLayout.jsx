...
export default function ProductLayout() {
  let {
    product,
    loader,
    pagenumberFilterState,
    totalPage,
    setPagenumberFilterState,
      ...
      <div class=" grid grid-cols-4 gap-3 relative">
        ...
        {product.length >= 1
          ? product.map((productItems, index) => (
              <ProductItems key={index} productData={productItems} />
            ))
          : "No Data Found"}
      </div>
      <div className="py-7">
...
function ProductItems({ productData }) {
  let { name, image, category_name, price, rating, discount_percentage } =
    productData;
  ...
  return (
    <div class="border-2 border-gray-300 border-solid ">
      ...
      <div class="flex justify-center pb-2">
        <button
          class="  border-2 border-[gray] rounded-md p-2 flex hover:bg-slate-200"
          fdprocessedid="nzllhd"
        >
          Add to cart
        </button>
      </div>
